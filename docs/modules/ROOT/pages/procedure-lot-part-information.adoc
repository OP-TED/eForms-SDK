...
"_cbc:RecurringProcurementIndicator_" element may be used with the value 
"false" to indicate that the Procurement will not be included in another 
Procedure.
...
To indicate that a particular awarding consequence does not apply, the 
respective element should be omitted.
...
To indicate that  _Electronic Ordering (BT-92)_ or _Electronic Payment (BT-93)_
shall not be used, the respective element should be used with the value 
"false".
...
the "_cbc:Description_" element. When Security Clearance is not required,
The element "_cac:SecurityClearanceTerm_" may be omitted, or used with the 
value "false" in the "_cbc:Code_" element.
...
The indicator should not be used with the value "false".
...
a Lot is covered by a Government Procurement Agreement or not. Where the
indicator is mandated by the regulation, the element 
"_cbc:GovernmentAgreementConstraintIndicator_" must be present.
