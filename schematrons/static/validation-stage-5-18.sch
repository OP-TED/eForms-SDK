...
<pattern id="EFORMS-validation-stage-5-18" xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <rule context="/*/cac:TenderingProcess">
        <assert id="R-GSB-RSM" role="ERROR" diagnostics="ND-ProcedureTenderingProcess_BT-105-Procedure" test="(cbc:ProcedureCode/normalize-space(text()) = ('restricted','neg-w-call')) or (not((cac:ProcessJustification[cbc:ProcessReasonCode/@listName='accelerated-procedure']/cbc:ProcessReasonCode/normalize-space(text()) = 'true') and (cbc:ProcedureCode)))">rule|text|R-GSB-RSM</assert>
    </rule>
    <rule context="/*">
