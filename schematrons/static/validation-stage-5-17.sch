...
<pattern id="EFORMS-validation-stage-5-17" xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <rule context="/*/cac:TenderingProcess">
        <assert id="R-FIG-XB6" role="ERROR" diagnostics="ND-ProcedureTenderingProcess_BT-105-Procedure" test="(cbc:ProcedureCode/normalize-space(text()) = ('open','restricted','neg-w-call','comp-dial')) or (not((cac:ProcessJustification[cbc:ProcessReasonCode/@listName='accelerated-procedure']/cbc:ProcessReasonCode/normalize-space(text()) = 'true') and (cbc:ProcedureCode)))">rule|text|R-FIG-XB6</assert>
    </rule>
    <rule context="/*">
