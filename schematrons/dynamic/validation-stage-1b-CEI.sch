...
<pattern id="EFORMS-validation-stage-1b-CEI" xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="/*/cac:ProcurementProjectLot[$noticeSubType = 'CEI']">
		...
		<assert id="ND-PartTenderingTerms-CEI" role="ERROR" diagnostics="ND-PartTenderingTerms" test="count(cac:TenderingTerms) &gt; 0">rule|text|ND-PartTenderingTerms-CEI</assert>
	</rule>
	<rule context="/*/cac:ProcurementProjectLot/cac:ProcurementProject[$noticeSubType = 'CEI']">
		...
		<assert id="ND-PartValueEstimate-CEI" role="ERROR" diagnostics="ND-PartValueEstimate" test="count(cac:RequestedTenderTotal) = 0">rule|text|ND-PartValueEstimate-CEI</assert>
	</rule>
	<rule context="/*/cac:ProcurementProjectLot/cac:TenderingProcess[$noticeSubType = 'CEI']">
		...
		<assert id="ND-PartPreviousPlanning-CEI" role="ERROR" diagnostics="ND-PartPreviousPlanning" test="count(cac:NoticeDocumentReference) = 0">rule|text|ND-PartPreviousPlanning-CEI</assert>
	</rule>
	...
	<rule context="/*/cac:ProcurementProjectLot/cac:TenderingTerms[$noticeSubType = 'CEI']">
		...
		<assert id="ND-PartEmploymentLegislation-CEI" role="ERROR" diagnostics="ND-PartEmploymentLegislation" test="count(cac:EmploymentLegislationDocumentReference) = 0">rule|text|ND-PartEmploymentLegislation-CEI</assert>
		<assert id="ND-PartEnvironmentalLegislation-CEI" role="ERROR" diagnostics="ND-PartEnvironmentalLegislation" test="count(cac:EnvironmentalLegislationDocumentReference) = 0">rule|text|ND-PartEnvironmentalLegislation-CEI</assert>
		<assert id="ND-PartFiscalLegislation-CEI" role="ERROR" diagnostics="ND-PartFiscalLegislation" test="count(cac:FiscalLegislationDocumentReference) = 0">rule|text|ND-PartFiscalLegislation-CEI</assert>
		<assert id="ND-PartProcurementDocument-CEI" role="ERROR" diagnostics="ND-PartProcurementDocument" test="count(cac:CallForTendersDocumentReference) &gt; 0">rule|text|ND-PartProcurementDocument-CEI</assert>
	...
	<rule context="/*/cac:TenderingProcess[$noticeSubType = 'CEI']">
		<assert id="ND-ProcedureFeaturesUnpublish-CEI" role="ERROR" diagnostics="ND-ProcedureFeaturesUnpublish" test="count(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy) = 0">rule|text|ND-ProcedureFeaturesUnpublish-CEI</assert>
	</rule>
