...
<pattern id="EFORMS-validation-stage-1b-17" xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="/*/cac:ProcurementProjectLot[cbc:ID/@schemeName='Lot']/cac:TenderingTerms[$noticeSubType = '17']">
		...
		<assert id="ND-FinancialGuarantee-17" role="ERROR" diagnostics="ND-FinancialGuarantee" test="count(cac:RequiredFinancialGuarantee) &gt; 0">rule|text|ND-FinancialGuarantee-17</assert>
	...
	<rule context="/*/cac:TenderingTerms[$noticeSubType = '17']">
		<assert id="ND-TendererQualificationRequest-17" role="ERROR" diagnostics="ND-TendererQualificationRequest" test="count(cac:TendererQualificationRequest) &gt; 0">rule|text|ND-TendererQualificationRequest-17</assert>
	</rule>
	...
	<rule context="/*/cac:TenderingTerms/cac:TendererQualificationRequest[$noticeSubType = '17']">
		<assert id="ND-ExclusionGrounds-17" role="ERROR" diagnostics="ND-ExclusionGrounds" test="count(cac:SpecificTendererRequirement) &gt; 0">rule|text|ND-ExclusionGrounds-17</assert>
	</rule>
	...
	<rule context="/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:AppealsInformation/efac:AppealStatus[$noticeSubType = '17']">
		<assert id="ND-AppealingParty-17" role="ERROR" diagnostics="ND-AppealingParty" test="count(efac:AppealingParty) = 0">rule|text|ND-AppealingParty-17</assert>
	</rule>
